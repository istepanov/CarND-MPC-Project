...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    const double change_accel_cost_weight = 100000000;

    fg[0] = 0;

    // Keep cte and epsi close to 0, keep speed close to reference speed
    ...
    for (size_t i = 1; i < N; i++) {
      ...
      fg[1 + epsi_start + i] = epsi_1 - (psi_0 - psides_0 - v_0 * delta_0 / Lf * dt);
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  double max_degree   = 25.0;
  double max_radians  = max_degree * M_PI / 180.0;
